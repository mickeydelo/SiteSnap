...
for (const page of config.pages) {
  for (const step of page.steps ?? []) step.enabled = VERIFY_STEP_IDS.has(step.id);
...
try {
  ...
  assert.equal(result.manifest.expectedCaptures, 6);
  assert.equal(result.manifest.completedCaptures, 6);
